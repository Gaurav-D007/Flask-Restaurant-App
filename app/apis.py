...
class CreateItemsOrderList(Schema):
    user_id = fields.Str(default = "user_id")
    item_id = fields.Str(default = "item_id")
    ...
    def post(self, **kwargs):
        if session['user_id']:
            ...
            if user_type == 1:
                ...
                db.session.add(item)
                db.session.commit()
                return APIResponse().dump(dict(message = 'Items are successful added')), 200
            else:
                return APIResponse().dump(dict(message = 'Logged in user is not Vendor')), 401
        else:
            return APIResponse().dump(dict(message = 'Please log-in')), 401
...
class CreateItemOrderAPI(MethodResource, Resource):
    ...
    @use_kwargs(CreateItemsOrderList, location='json')
    @marshal_with(APIResponse)
    def post(self, **kwargs):
        try:
            if session.get('user_id'):
                ...
                if user_type == 0:
                    order_id = uuid.uuid4()
                    order = Order(order_id=order_id, user_id=user_id)  # Corrected order_id argument
                    db.session.add(order)
                    for item in kwargs['items']:
                        item = dict(item)
                        order_item = OrderItems(
                            order_item_id=uuid.uuid4(),  # Corrected argument names
                            order_id=order_id,
                            item_id=item['item_id'],  # Corrected key name
                            quantity=item['quantity']  # Corrected key name
                        )
                        db.session.add(order_item)
                    db.session.commit()
                    return APIResponse().dump(dict(message=f'Items of order are successfully added for order_id: {order_id} ')), 200
                else:
...
class PlaceOrderAPI(MethodResource, Resource):
    ...
    def post(self, **kwargs):
        try:
            if session.get('user_id'):
                ...
                if user_type == 0:
                    ...
                    for order_item in order_items:
                        ...
                        print(total_amount)
                        item.available_quantity -= quantity  # Updated available_quantity calculation
            ...
            print(str(e))
            return APIResponse().dump(dict(message=f'Not able to place order: {str(e)}')), 4003
